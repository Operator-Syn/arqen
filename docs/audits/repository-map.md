...
| `src/gmail/` | Gmail REST models, validation, client, and response mapping | label list, bounded metadata list, and bounded full-message text read |
| `src/broker/` | Unix credential broker/client, framing, handlers, and cache | selected target, eligibility, refresh, stable errors |
| `src/mcp.rs` | Broker wire types | label-list/list/readiness/read operations and result/error serialization |
| `src/server/` | Streamable HTTP MCP process, auth, routes, and runtime | bearer/Host/Origin gate and label-list/list/read tools |
