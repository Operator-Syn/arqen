...
the default TUI, `credential-broker`, and `mcp-server`. The MCP server never
...
| --- | --- | --- |
| TUI (`src/main.rs`, `src/ui/`) | Account login/logout, target selection, presentation | MCP HTTP, remote auth, access-token caching |
